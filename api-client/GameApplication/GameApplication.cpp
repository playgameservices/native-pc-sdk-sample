...
#define HELPER_CLASS_NAME \
  "com/dynasty/pconlygame/NDKHelper"  // Class name of helper function
//-------------------------------------------------------------------------
...
namespace GameApplication {
    ...
    void GameApplication::RunListener(Object^ listenerObj) {
        HttpListenerThread^ listerThread = dynamic_cast<HttpListenerThread^>(listenerObj);
        if (listerThread != nullptr) {
            listerThread->StartListening();
        }
    }
...
public:
    GameForm()
    {
        this->Text = "OpenGL with GLEW and FreeGLUT in Windows Forms";
        this->Size = System::Drawing::Size(700, 500);
        CGLControl^ openGLControl = gcnew CGLControl();
        openGLControl->Dock = DockStyle::Fill;
        this->Controls->Add(openGLControl);

        // Initialize OpenGL
        openGLControl->InitializeOpenGL();
    }
...
int main(array<System::String^>^ args)
{
    ...
    // Run the application
    System::Windows::Forms::Application::Run(gcnew GameForm());
